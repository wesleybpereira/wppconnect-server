...
if [ -z "$PUPPETEER_EXECUTABLE_PATH" ]; then
    ...
    # Se não encontrar, tentar no Nix store
    if [ -z "$PUPPETEER_EXECUTABLE_PATH" ]; then
        NIX_CHROMIUM=$(find /nix/store -name "chromium" -type f -executable 2>/dev/null | head -1)
        if [ -n "$NIX_CHROMIUM" ]; then
            export PUPPETEER_EXECUTABLE_PATH="$NIX_CHROMIUM"
            echo "✅ Chromium encontrado no Nix: $NIX_CHROMIUM"
        fi
    fi
fi
